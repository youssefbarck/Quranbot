...
async def set_initial_hifz(session: AsyncSession, user: User, last_page: int) -> None:
    ...
    user.weekly_prep_start = last_page + 1 if last_page < config.QURAN_PAGE_COUNT else last_page
    user.weekly_prep_end = min(
        config.QURAN_PAGE_COUNT,
        user.weekly_prep_start + (user.weekly_hifz_amount or config.DEFAULT_WEEKLY_HIFZ_AMOUNT) - 1,
    )
